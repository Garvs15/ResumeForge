...
export function CreateBaseResumeDialog({ children, profile }: CreateBaseResumeDialogProps) {
    ...
    const handleItemSelection = (section: keyof typeof selectedItems, id: string) => {
        setSelectedItems(prev => ({
            ...prev,
            [section]: prev[section].includes(id) ? prev[section].filter(x => x !== id)
                : [...prev[section], id]
        }));
    };
    ...
    const handleSectionSelection = (section: keyof typeof selectedItems, checked: boolean) => {
        setSelectedItems(prev => ({
            ...prev,
            [section]: checked ? profile[section].map((item, index) => getItemId(section, item, index))
                : []
        }));
    };
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            if (importOption === 'import-resume') {
                ...
                // Get Model and API key from the local storage
                const MODEL_STORAGE_KEY = 'resumeforge-default-model';
                const LOCAL_STOARGE_KEY = 'resumeforge-api-keys';
                const selectedModel = localStorage.getItem(MODEL_STORAGE_KEY);
                const storedKeys = localStorage.getItem(LOCAL_STOARGE_KEY);
                let apiKeys = [];
                try {
                    apiKeys = storedKeys ? JSON.parse(storedKeys) : [];
                } catch (error) {
                    console.error('Error parsing the API Keys:', error);
                }
            ...
            const selectedContent = {
                work_experience: profile.work_experience.filter((exp, index) => selectedItems.work_experience.includes(getItemId('work_experience', exp, index))
                ),
                education: profile.education.filter((edu, index) => selectedItems.education.includes(getItemId('education', edu, index))),
                skills: profile.skills.filter((skill, index) => selectedItems.skills.includes(getItemId('skills', skill, index))),
                projects: profile.projects.filter((project, index) => selectedItems.projects.includes(getItemId('projects', project, index)))
            };
    ...
    // Reset form and initialize the selected items when dialog opens
    ...
    const extractTextFromPDF = async (file: File) => {
        ...
        // console.log("📦 PDFJS Loaded:", pdfjsLib);
    ...
    const handleFileInput = async (e: React.ChangeEvent<HTMLInputElement>) => {
        console.log(1)
        console.log(e.target.files?.[0], 'aasad')
        const file = e.target.files?.[0];
        if (file && file.type === "application/pdf") {
            try {
                const text = await pdfToText(file);
                console.log(text, 'asaaas')
    ...
    const handleDrop = async (e: React.DragEvent) => {
        ...
        if (!pdfFile) {
            console.error("❌ No PDF found in dropped files");
            return;
        }
    ...
    //         console.error("❌ Not a PDF:", file.type);
    //         return;
    //     }
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
          ...
          .shake {
            animation: shake 0.8s cubic-bezier(.36,.07,.19,.97) both;
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            ...
                            <DialogDescription className="text-sm text-gray-600">
                                {currentStep === 1 ? "Start by entering your target role"
                ...
                {/* Content */}
                <div className="py-6 px-6 min-h-[300px]">
                    {currentStep === 1 && (
                ...
                {/* Error Dialog */}
                <ApiErrorDialog open={showErrorDialog}
