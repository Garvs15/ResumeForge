...
import { CustomPrompts } from "@/lib/types";
import { useCallback, useSyncExternalStore } from "react";

// Storage key for custom prompts
const CUSTOM_PROMPTS_STORAGE_KEY = 'resumeforge-custom-propmts';
...
// In memory cache to keep snapshots stable b/w calls
...
function handleStorageChange(event: StorageEvent) {
    if (event.key === CUSTOM_PROMPTS_STORAGE_KEY) {
        const parsed = parseCustomPropmts(event.newValue);
...
function readStoredPrompts(): CustomPrompts {
    ...
    return parseCustomPropmts(stored);
}

function parseCustomPropmts(raw: string | null): CustomPrompts {
...
// Snapshots function - return stable references
...
export function useCustomPrompts() {
    ...
    const setCustomPrompts = useCallback((updater: CustomPrompts | ((prev: CustomPrompts) => CustomPrompts)) => {
        ensureClientStoreInitialized();
    ...
    const setPrompt = useCallback((key: keyof CustomPrompts, value: string | undefined) => {
    ...
    const resetPrompt = useCallback((key: keyof CustomPrompts) => {
        setCustomPrompts(prev => {
            const { [key]: _removed, ...rest } = prev;
            return rest;
        })
