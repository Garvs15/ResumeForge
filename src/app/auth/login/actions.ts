...
export async function signup(formData: FormData): Promise<AuthResult> {
    ...
    if (signupData.user && AUTO_PRO_SUBSCRIPTION) {
        ...
        if (subscriptionError) {
            ...
            // Don't fail signup is subscription creation fails
        }
    }
...
export async function joinWaitlist(formData: FormData): Promise<AuthResult> {
    ...
    try {
        ...
        return {
            success: false,
            error: e instanceof Error ? e.message : 'An unexpected error occured'
        };
    }
}
...
// New function to check the subscription status
