        ...
        <div className="space-y-2">
            ...
            <div className="space-y-1.5">
                {work.description.map((point, index) => {
                    ...
                    const comparedWords = currentPoint ? compareDescriptions(currentPoint, point)
            ...
            {project.technologies && (
                <div className="flex flex-wrap gap-1.5 mt-2">
                    {project.technologies.map((tech, techIndex) => (
                        <span key={techIndex} className={cn(
            ...
            <div className="flex justify-between items-start">
                <div>
                    <h3 className={cn(
                        ...
                        <span>
                            {education.degree.split(/(\*\*.*?\*\*)/).map((part, i) => part.startsWith('**') && part.endsWith('**')
                                ? <strong key={i}>{part.slice(2, -2)}</strong> : part)}
                        </span>
                        {' in '}
                        <span>
                            {education.field.split(/(\*\*.*?\*\*)/).map((part, i) => part.startsWith('**') && part.endsWith('**')
                                ? <strong key={i}>{part.slice(2, -2)}</strong> : part)}
                        </span>
                    </h3>
                    <p className={cn(
            ...
            {education.achievements && (
                <div className="space-y-1.5">
                    {education.achievements.map((achievement, index) => {
                        ...
                        const comparedWords = currentAchievement ? compareDescriptions(currentAchievement, achievement)
                            : [{ text: achievement.replace(/\*\*/g, ''), isNew: true, isBold: false, isStart: true, isEnd: true }];

                        return (
                            <div key={index} className="flex items-start gap-1.5">
                                <span className="text-gray-800 mt-0.5 text-xs">•</span>
                                <p className="text-xs text-gray-800 flex-1 flex-wrap">
                                    {comparedWords.map((word, wordIndex) => (
                                        <span key={wordIndex} className={cn(
                                            "inline-flex items-center",
                                            word.isNew && "bg-green-300",
...
function compareDescriptions(current: string, suggested: string): {
    ...
    // Clean the text by normalizing spaces and removing extra whitespaces
    ...
    return suggestedWords.map((word, index) => {
        ...
        // Check if the word exists in the current text (considering the bold text)
        ...
        // Check if the adjacent words are new
...
export function Suggestion({ type, content, currentContent, onAccept, onReject }: SuggestionProps) {
    ...
    // Helper function to get style-based styles
