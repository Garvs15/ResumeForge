...
export function SavedStyleDialog({ currentSettings, onApplyStyle }: SavedStylesDialogProps) {
    ...
    useEffect(() => {
        const saved = localStorage.getItem("resumeforge-saved-styles");
        if (saved) {
            setSavedStyles(JSON.parse(saved));
        }
    }, []);

    // Saved current settings with name
    const handleSaveStyle = () => {
        ...
        localStorage.setItem("resumeforge-saved-styles", JSON.stringify(updatedStyles));
        setNewStyleName("");
        ...
        backdrop-blur-2xl border-white/60 shadow-2xl pt-12">
                <DialogHeader>
                    <div className="flex items-center justify-between">
                        <DialogTitle className="text-xl bg-gradient-to-r from-teal-700 to-cyan-700 bg-clip-text text-transparent font-semibold">
                            ...
                            <div className="p-4 space-y-3">
                                {savedStyles.length === 0 ? (
                                    ...
                                    savedStyles.map((style) => (
                                        <div key={style.timestamp} className="flex items-center justify-between group rounded-xl border 
