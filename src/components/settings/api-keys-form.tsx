...
import { AI_MODELS, ApiKey, getProvidersArray, MODEL_DESIGNATIONS } from "@/lib/ai-models";
...
export function ApiKeysForm({ isProPlan }: { isProPlan: boolean }) {
    ...
    // Track if have initialized the default model
    const hasInitialized = useRef(false);

    //    Initialize the default model if not set (only runs once on mount)
    useEffect(() => {
    ...
    const handleUpdateKey = (service: ServiceName) => {
        ...
        // Automatically set default model based on the provider
    ...
    const handleRemoveKey = (service: ServiceName) => {
        ...
        // Check if current default model requires the API key
        const currentModel = AI_MODELS.find(m => m.id === defaultModel);
        if (currentModel?.provider === service) {
            // Find first available model that has the API key
            const firstAvailableModel = AI_MODELS.find(m => apiKeys.some(k => k.service === m.provider && k.service !== service));
                ...
                <div className="space-y-4">
                    ...
                    <div className="mt-8 pt-6 border-t border-amber-200/50">
                        <div className="mb-4 p-3 rounded-lg bg-amber-50/50 border border-amber-200/50 text-amber-900 text-sm">
                            <p className="font-medium">Experiemental Providers Notice</p>
