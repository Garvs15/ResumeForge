...
export async function checkSubscriptionPlan() {
    ...
    const hasManualProAccess = data?.subscription_plan === 'pro' && data.subscription_status === 'active';
    const hasStripeTimeboxedAccess = hasStripeSubscription && isWithinAccessWindow;
    const hasCancelingProAccess = data?.subscription_plan === 'pro' &&
...
// Check if user has ever started a subscription/trial (user for getting)
