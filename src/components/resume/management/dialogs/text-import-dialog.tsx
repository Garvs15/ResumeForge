...
export function TextImportDialog({
    ...
    const handleDrag = async (e: React.DragEvent) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const extractTextFromPDF = async (file: File) => {
         console.log("🚀 Using react-pdf pdfjs");
  ...
  `//cdnjs.cloudflare.com/ajax/libs/pdf.js/${pdfjs.version}/pdf.worker.min.js`;
    ...
    const handleImport = async () => {
        ...
        try {
            const updateResume = await addTextToResume(content, resume);

            // Update each field of the resume
            (Object.keys(updateResume) as Array<keyof Resume>).forEach((key) => {
                onResumeChange(key, updateResume[key] as Resume[keyof Resume]);
            });
                ...
                <DialogHeader>
                    ...
                    <DialogDescription asChild>
                        <div className="space-y-2 text-base text-muted-foreground/80">
                            ...
                            <ol className="list-decimal list-inside spae-y-1 ml-1">
