...
interface ResumeEditorActionProps {
    onResumeChange: (field: keyof Resume, value: Resume[keyof Resume]) => void;
}

export function ResumeEditorActions({
    onResumeChange
}: ResumeEditorActionProps) {
                                    ...
                                    try {
                                        ...
                                        if (downloadOptions.coverLetter && resume.has_cover_letter) {
                                            ...
                                            if (!coverLetterElement) {
                                                throw new Error('Cover Letter content is not found!');
                                            }

                                            const opt = {
                                                margin: [0, 0, -0.5, 0] as [number, number, number, number],
                            ...
                            <div className="space-y-3">
                                <label className="flex items-center space-x-2">
                                    <Checkbox
                                        ...
                                        onCheckedChange={(checked) => setDownloadOptions(prev => ({ ...prev, resume: checked as boolean }))}
                                        className={cn(
                                            resume.is_base_resume
                                ...
                                <label className="flex items-center space-x-2">
                                    <Checkbox
                                        ...
                                        onCheckedChange={(checked) => setDownloadOptions(prev => ({ ...prev, coverLetter: checked as boolean }))}
                                        className={cn(
                                            resume.is_base_resume
                                                ? "border-indigo-400 data-[state=checked]:bg-indigo-600 data-[state=checked]:border-indigo-600"
                                                : "border-rose-400 data-[state=checked]:bg-rose-600 data-[state=checked]:border-rose-600"
