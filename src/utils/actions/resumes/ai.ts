...
import { AIConfig, Resume, WorkExperience } from "@/lib/types";
import { getSubscriptionPlan } from "../stripe/actions";
import { getDefaultModel } from "@/lib/ai-models";
import { generateObject, type LanguageModel } from "ai"; // Renamed LanguageModelV1 -> LanguageModel in newer versions
...
export async function convertTextToResume(prompt: string, existingResume: Resume, targetRole: string, config?: AIConfig) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        console.error("AI validation failed:", err?.issues || err);
        throw new Error("Failed to convert text to resume: AI output did not match schema.");
    }
...
export async function generateWorkExperiencePoints(
    ...
    const systemPrompt = config?.customPrompts?.workExperienceGenerator ?? (WORK_EXPERIENCE_GENERATOR_MESSAGE.content as string);
...
export async function improveProject(point: string, customPrompt?: string, config?: AIConfig) {
    ...
    // Use the custom prompt if provided in config, otherwise fall back to default
