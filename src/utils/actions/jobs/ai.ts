...
// Build Model Candidates list - priortize user's selected model if provided
...
export async function tailorResumeToJob(
    ...
    for (const candidate of modelCandidates) {
        ...
        try {
            start = Date.now();
            console.log(`[TAILOR][TRY] ${candidate.model} | STEP: Tailoring resume content | Subscription: ${isPro ? 'PRO' : 'FREE'}`);
...
Your task: produce a polished, tailored resume that meets the schema exactly and reads like a refined human-written resume, not a template with explicit STAR labels.
    ...
    `,
            ...
            console.log(`[TAILOR][SUCCESS ✅] ${candidate.model} | Duration: ${Date.now() - start}ms | STEP: Tailoring resume content`);
            return object.content satisfies z.infer<typeof simplifiedResumeSchema>;
        } catch (error) {
            lastError = error;
            console.error(`[TAILOR][FAILED ❌] ${candidate.model} | STEP: Tailoring resume content | Duration: ${Date.now() - start}ms | Reason: ${(error as Error)?.message ?? 'Unknown error'}`, error);
        }
    }
...
export async function formatJobListing(jobListing: string, config?: AIConfig) {
    ...
    // Check the rate limit once per formatting request
    await checkRateLimit(id);
    ...
    throw lastError ?? new Error('Failed to format the job listing!');
}