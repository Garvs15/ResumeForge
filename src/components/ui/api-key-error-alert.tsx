...
export function ApiKeyErrorAlert({ error, router }: ApiKeyErrorAlertProps) {
    ...
    try {
        console.log('Error signified:', JSON.stringify(error, null, 2));
    } catch (error) {
        console.error('Error cannot be signified', error);
    }
            ...
            <div className={cn(
                ...
                <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="font-medium text-red-600">
                        {typeof error === 'string' ? error : ((error as Error)?.name === 'DataCloneError' || (error as Error)?.message?.includes('structuredClone')
                ...
                {((error as Error)?.message?.includes('API key') ||
                    ...
                    JSON.stringify(error).includes('authentication_error')) &&
                    !((error as Error)?.name === 'DataCloneError' || (error as Error)?.message?.includes('structuredClone') ||
                        (error as Error)?.message?.includes('could not be cloned')) ? (
                    <>
