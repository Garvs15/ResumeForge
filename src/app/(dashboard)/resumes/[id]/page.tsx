...
function normalizeResumeData(resume: Resume): Resume {
    return {
        ...
        // Initialize document settings with default if not present
...
export async function generateMetadata({
    ...
    try {
        ...
        return {
            title: `${resume.name} | ResumeForge`,
            description: `Editing ${resume.name} - ${resume.target_role} resume`
        };
    } catch (error) {
        void error;
        return {
            title: 'Resume Editor | ResumeForge',
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        const data = await getResumePageData(id);
        ...
        normalizedResume = normalizeResumeData(data.resume);
