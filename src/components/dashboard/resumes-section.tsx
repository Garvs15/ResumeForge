...
import { ResumeSortControls, SortDirection, SortOption } from "../resume/management/resume-sort-controls";
import { useTrialGate } from "../trial/trial-gate";
...
export function ResumeSection({
    ...
    const [optimisticResumes, removeOptimisticResume] = useOptimistic(
        ...
        (state, deleteResumeId: string) => state.filter(resume => resume.id !== deleteResumeId)
    );
    ...
    const handleCopyResume = async (sourceResume: OptimisticResume) => {
        ...
        try {
            // Call the server action in background
            ...
            // On error, the optimistic update will automatically rollback
            console.error("Failed to copy the resume:", error);
    ...
    const CreateResumeCard = () => (
        ...
        //             "transform transition-all duration-all",
        ...
        //             )}>
        //                 <config.icon className={cn("h-5 w-5 transition-all duration-500",
