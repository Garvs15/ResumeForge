...
export const jobSchema = z.object({
    ...
    exployment_type: z.preprocess((val) => val === null || val === '' ? 'full_time' : val,
        z.enum(['full_time', 'part_time', 'co_op', 'internship', 'contract']).default('full_time').optional()),
...
export const simplifiedJobSchema = z.object({
    ...
    work_location: z.preprocess((val) => val === null || val === '' ? 'in_person' : val,
...
export const workExperienceBulletPointsSchema = z.object({
    ...
    analysis: z.object({
        impact_score: z.number().min(1).max(0).describe("Score indicating the overall impact of these achievements (1-10)"),
