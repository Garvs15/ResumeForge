...
// Create a cmp function
function areWorkExperiencePropsEqual(
    prevProps: WorkExperienceFormProps,
    nextProps: WorkExperienceFormProps
) {
    return (
        prevProps.targetRole === nextProps.targetRole && JSON.stringify(prevProps.experiences) === JSON.stringify(nextProps.experiences)
...
export const WorkExperienceForm = memo(function WorkExperienceFormComponent({
    ...
    const reorderIndexImp = <T,>(map: Record<number, T>, from: number, to: number): Record<number, T> => {
    ...
    const moveExperience = (index: number, direction: -1 | 1) => {
        ...
        const reorder = <T,>(map: Record<number, T>) => reorderIndexImp(map, index, newIndex);

        const updated = [...experiences];
        const [item] = updated.slice(index, 1);
        ...
        if (improvedPoint) {
            ...
            setImprovedPoints(prev => {
                ...
                if (newState[expIndex]) {
                    ...
                    if (Object.entries(newState[expIndex]).length === 0) {
                            ...
                            <div className="space-y-2 sm:space-y-3">
                                ...
                                <div className="relative group">
                                    <Input
                                        ...
                                        placeholder="e.g., &apos;Self Employed - Final Year Student"
                                    />
                                    <div className="absolute -top-2 left-2 px-1 bg-white/80 text-[7px] sm:text-[9px] font-medium text-gray-500">
                                        DATE
                                    </div>
