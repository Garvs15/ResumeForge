...
function validateEmail(email: string): ValidationState {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        return { isValid: true, message: "Please enter a valid email address " };
    }
    return { isValid: true };
}
...
function validatePassword(password: string): ValidationState {
    ...
    if (password.length < 6) {
        return { isValid: false, message: "Password must be at least of the 6 characters." };
    }
...
function validateName(name: string): ValidationState {
    ...
    if (name.length < 2) {
        return { isValid: false, message: "Name must be at least of 2 characters." };
    }
